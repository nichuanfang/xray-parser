...
def update_config():
    ...
    with open('../config/config.json','rb') as config_file:
        ...
        with open('routing/black/routing_header.json') as routing_header:
            rh = json.load(routing_header)

        with open('routing/black/routing_body.json') as routing_body:
            rb = json.load(routing_body)

        with open('routing/black/routing_footer.json') as routing_footer:
        ...
        with open('dist/trojan.txt','w+') as trojan:
            trojan.writelines(f'trojan={VLESS_CLIENT_SERVER_NAME}:{TROJAN_PORT}, password={TROJAN_PASSWORD}, over-tls=true, tls-verification=true, fast-open=false, udp-relay=false, tag=mysub')
